...
SELECT numID,'1','1',robNo,NULL FROM `CQ2_number` WHERE robNo='R001' AND callState='1' 
...
--
DELIMITER $$
CREATE TRIGGER `AddFrssDes2` AFTER INSERT ON `on_duty_designer` FOR EACH ROW INSERT INTO waitingdes(`desNo` ,`desState` ,`finishTime` ,`stoNo`) 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `addFrssDes` AFTER UPDATE ON `on_duty_designer` FOR EACH ROW INSERT INTO waitingdes(`desNo` ,`desState` ,`finishTime` ,`stoNo`) 
SELECT `desNo` ,`desState` ,`finishTime` ,`stoNo` FROM on_duty_designer 
WHERE stoNo='1'
 AND desState='0'ORDER BY finishTime DESC LIMIT 0,1
$$
